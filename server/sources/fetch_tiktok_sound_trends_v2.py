...
SOURCE = f"tiktok:sound:v2:{COUNTRY}"
JSON_PATH = os.getenv("TIKTOK_SOUND_TRENDS_JSON_PATH", "").strip()

FALLBACK_ITEMS = [
    {"term": "girl dinner aesthetic", "sound": "girl dinner", "score": 90},
    {"term": "pink bow outfit", "sound": "coquette pop", "score": 87},
    {"term": "cat cafe chill", "sound": "soft jazz cat", "score": 85},
    {"term": "teacher morning routine", "sound": "school day reset", "score": 84},
    {"term": "nurse shift humor", "sound": "night shift laugh", "score": 82},
]
...
def main():
    ...
    for idx, item in enumerate(items, start=1):
        term = norm(item.get("term") or item.get("sound"))
        if not term:
            continue
        ...
        cur.execute("""INSERT OR IGNORE INTO raw_trends
            ...
            (dstr(), COUNTRY, term, score, SOURCE,
             ...
             json.dumps({"fetcher":"fetch_tiktok_sound_trends_v2.py"}, ensure_ascii=False),
             dedup, utc()))
        if cur.rowcount > 0:
            raw_inserted += 1
        cur.execute("""INSERT INTO tiktok_trend_signals
            ...
            VALUES (?,?,?,?,?,?,?)""",
            (SOURCE, "sound", term, score, "", json.dumps(item, ensure_ascii=False), utc()))
        signal_inserted += 1
    conn.commit()
    print(f"[OK] fetch_tiktok_sound_trends_v2 raw_inserted={raw_inserted} signals={signal_inserted} db={DB_PATH}")
    conn.close()
