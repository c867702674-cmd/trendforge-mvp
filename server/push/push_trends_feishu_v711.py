...
# -*- coding: utf-8 -*-
"""
TrendForge Push Engine V7.1.1
- 基于原 push_trends_feishu.py 主干能力
- 保留：多群、digest/detail、cooldown、push_log、dry-run、ignore-cooldown、Execution Pack、按钮
- 接入 V7 数据链路：design_ideas / design_prompts
- 增强：飞书返回值打印、失败日志、interactive 失败自动 fallback 到 text
...
Usage:
  python3 push_trends_feishu_v711.py --dry-run 1
  python3 push_trends_feishu_v711.py --dry-run 0
  python3 push_trends_feishu_v711.py --ignore-cooldown 1 --dry-run 0
"""
...
def ensure_v7_tables_soft(conn: sqlite3.Connection) -> None:
    # Soft-ensure only. Do not downgrade existing richer schemas.
    ...
    if not table_exists(conn, "design_prompts"):
        conn.execute(
            """
...
def a3_try_auto_expand(conn: sqlite3.Connection, trend_id: int, term: str, n: int) -> bool:
    ensure_v7_tables_soft(conn)
    if a3_has_ideas(conn, trend_id):
        return True
    term = (term or "").strip()
    if not term:
        return False
    # Optional compatibility with older expansion_engine.py
    try:
...
def fetch_design_ideas(conn: sqlite3.Connection, trend_id: int, limit: int = 8) -> List[str]:
    ...
    if column_exists(conn, "design_ideas", "score"):
        order_sql = "ORDER BY COALESCE(score,0) DESC, id ASC"
    rows = conn.execute(
        f"SELECT idea FROM design_ideas WHERE trend_id=? {order_sql} LIMIT ?;",
        (trend_id, limit),
    ).fetchall()
    out: List[str] = []
    for r in rows:
...
def fetch_mj_prompts(conn: sqlite3.Connection, trend_id: int, limit: int = 1) -> List[str]:
    ...
    if table_exists(conn, "design_prompts") and table_exists(conn, "design_ideas"):
        order_sql = "ORDER BY dp.id DESC"
        if column_exists(conn, "design_prompts", "score"):
            order_sql = "ORDER BY COALESCE(dp.score,0) DESC, dp.id ASC"
        rows = conn.execute(
            f"""
            SELECT dp.prompt
...
def _render_exec_pack_md(pack: Optional[Dict[str, Any]], show_skus: int, show_bullets: int, show_bk: int) -> str:
...
def build_digest_card(trends: List[sqlite3.Row], group: GroupCfg, web: str, boost_weight: float) -> Dict[str, Any]:
    items: List[str] = []
    for t in trends:
    ...
    card = {
        ...
        "header": {"title": {"tag": "plain_text", "content": f"🧾 DIGEST · {group.name}"}, "template": group.template},
        "elements": [
...
def build_digest_text(trends: List[sqlite3.Row], group: GroupCfg, web: str, boost_weight: float) -> Dict[str, Any]:
    lines = [f"🧾 DIGEST · {group.name}", ""]
    for i, t in enumerate(trends, start=1):
        ...
        boost = float(row_get(t, "feedback_boost_score", 0) or 0)
        final = calc_final(hit, boost, boost_weight)
        lines.append(f"{i}. #{tid} {term} · final {final:.0f} · {web.rstrip('/')}/?trend_id={tid}")
    if len(lines) == 2:
        lines.append("（本轮无可推送条目）")
    return {"msg_type": "text", "content": {"text": "\n".join(lines)}}


def build_detail_card_v711(
    trend: sqlite3.Row,
    group: GroupCfg,
    web: str,
    boost_weight: float,
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": f"🔥 {lvl} · #{tid} · {group.name}"},
...
def build_detail_text(
    ...
    lines = [
        f"🔥 {group.name}",
        f"Trend: #{tid} {term}",
        f"{country} · {category} · final {final:.0f}",
        "",
        "Design Ideas:",
    ]
    if ideas:
        ...
        lines.append("（暂无扩散词）")
    lines.append("")
    lines.append("MJ Prompt:")
    if mj_prompts:
        lines.append(mj_prompts[0])
    else:
        lines.append("（暂无 MJ Prompt）")
    lines.append("")
    lines.append(f"详情: {web.rstrip('/')}/?trend_id={tid}")
    return {"msg_type": "text", "content": {"text": "\n".join(lines)}}
...
def parse_feishu_resp(http_status: int, body: str) -> Tuple[int, Optional[int], Optional[str], str]:
    fei_code = None
    fei_msg = None
    ok = 0
    try:
        ...
        if isinstance(bj, dict):
            fei_code = bj.get("code")
            fei_msg = bj.get("msg")
        ok = 1 if (http_status == 200 and (fei_code in (0, None))) else 0
    except Exception:
        ok = 1 if http_status == 200 else 0
    return ok, fei_code, fei_msg, body or ""


def post_with_fallback(url: str, payload: Dict[str, Any], fallback_payload: Optional[Dict[str, Any]] = None) -> Tuple[int, int, Optional[int], Optional[str], str, str]:
    # returns ok, http_status, feishu_code, feishu_msg, body, err_stage
    http_status, body = http_post_json(url, payload)
    ok, fei_code, fei_msg, body2 = parse_feishu_resp(http_status, body)
    if ok:
        return 1, http_status, fei_code, fei_msg, body2, ""
    if fallback_payload is not None:
        http_status2, body_f = http_post_json(url, fallback_payload)
        ok2, fei_code2, fei_msg2, body_f2 = parse_feishu_resp(http_status2, body_f)
        if ok2:
            return 1, http_status2, fei_code2, fei_msg2, body_f2, "fallback_text"
        return 0, http_status2, fei_code2, fei_msg2, body_f2, "fallback_failed"
    return 0, http_status, fei_code, fei_msg, body2, "interactive_failed"
...
def main() -> int:
    ...
    ap.add_argument("--db", default=env_str("DB_PATH", env_str("TRENDFORGE_DB", "/root/trendforge-mvp/server/trendforge.db")))
    ...
    vip_auto_expand = env_bool("VIP_AUTO_EXPAND", False)
    ...
    print(f"[INFO] V7.1.1 auto_expand={vip_auto_expand} show_ideas={vip_show_ideas} show_prompts={vip_show_prompts}")
    print(f"[INFO] V7.1.1 exec_pack enable={vip_enable_exec_pack} cache_hours={exec_pack_cache_hours} show_skus={vip_show_skus}")
    ...
    for g0 in groups:
        ...
        if g.mode == "digest":
            ...
            fallback_payload = build_digest_text(picked, g, web, boost_weight)
            ...
            if args.dry_run:
                ...
                continue
            ok, http_status, fei_code, fei_msg, body, stage = post_with_fallback(url, payload, fallback_payload)
            print(f"[SEND] {g.key} digest ok={ok} http={http_status} feishu_code={fei_code} stage={stage or 'interactive'}")
            for t in picked:
                log_push(conn, g.key, g.name, url, mh, ok, http_status, fei_code, fei_msg, body, None if ok else stage, int(row_get(t, 'id', 0)))
            if ok:
                pushed_total += len(picked)
            else:
        ...
        for t in picked:
            ...
            payload = build_detail_card_v711(
                trend=t,
                group=g,
                web=web,
            ...
            fallback_payload = build_detail_text(t, g, web, boost_weight, ideas, mj_prompts)
            mh = sha1(json.dumps(payload, ensure_ascii=False, sort_keys=True))
            ...
            ok, http_status, fei_code, fei_msg, body, stage = post_with_fallback(url, payload, fallback_payload)
            print(f"[SEND] {g.key} id={tid} ok={ok} http={http_status} feishu_code={fei_code} stage={stage or 'interactive'}")
            log_push(conn, g.key, g.name, url, mh, ok, http_status, fei_code, fei_msg, body, None if ok else stage, tid)
            if ok:
                pushed_total += 1
            else:
                failed_total += 1
