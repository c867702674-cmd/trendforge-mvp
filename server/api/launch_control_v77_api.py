...
OUT_PATH = os.path.join(BASE_DIR, "docs", "launch_control_v77.json")
...
def main():
    ...
    for r in rows:
        by_lane[r["control_lane"]] += 1
        by_product[r["product_type"]] += 1
        by_priority[r["priority_tier"]] += 1
        items.append({
            "id": r["id"],
            "launch_orchestrator_id": r["launch_orchestrator_id"],
            "dispatch_center_id": r["dispatch_center_id"],
            "release_routing_id": r["release_routing_id"],
            ...
            "orchestrator_lane": r["orchestrator_lane"],
            "orchestrator_score": r["orchestrator_score"],
            "control_lane": r["control_lane"],
    ...
    payload = {
        "ok": True,
        "version": "v77",
        "module": "launch_control",
        "summary": {
            "total": len(items),
            "by_control_lane": dict(by_lane),
    ...
    print(f"[OK] launch_control_v77_api wrote={OUT_PATH} items={len(items)}")
    conn.close()
