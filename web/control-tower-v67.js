...
function renderSummary(summary){
  ...
  const parts = [`<div class="card"><div>总控制塔</div><strong>${summary.total || 0}</strong></div>`];
  const byLane = summary.by_tower_lane || {};
...
function cls(lane){
  if(lane === 'LAUNCH_TOWER') return 'launch';
  if(lane === 'ART_TOWER') return 'art';
  if(lane === 'POSTER_TOWER') return 'poster';
  if(lane === 'BACKLOG_TOWER') return 'backlog';
  return 'manual';
}
...
function renderGrid(items){
  const mapping = {
    'LAUNCH_TOWER':'col-launch',
    'ART_TOWER':'col-art',
    'POSTER_TOWER':'col-poster',
    'BACKLOG_TOWER':'col-backlog',
    'MANUAL_TOWER':'col-manual',
  };
  ...
  for (const [group, id] of Object.entries(mapping)) {
    const html = items.filter(x=>x.tower_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items = RAW.filter(x=>{
    const hay = [x.title_en, x.sku_code, x.safe_term, x.tower_action].join(" ").toLowerCase();
...
fetch("/docs/control_tower_v67.json")
  .then(r=>r.json())
  .then(data=>{
    RAW = data.items || [];
