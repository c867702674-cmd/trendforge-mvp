...
function renderSummary(summary){
  ...
  const parts = [`<div class="card"><div>总调度中心</div><strong>${summary.total || 0}</strong></div>`];
  const byLane = summary.by_dispatch_lane || {};
...
function cls(lane){
  if(lane === 'FAST_DISPATCH') return 'fast';
  if(lane === 'CREATIVE_DISPATCH') return 'creative';
  if(lane === 'POSTER_DISPATCH') return 'poster';
  if(lane === 'RESERVE_DISPATCH') return 'reserve';
  return 'manual';
}
  ...
  return `
    <div class="item">
      ...
      <div class="meta">
        ...
        <span class="badge ${cls(x.dispatch_lane)}">${esc(x.dispatch_lane)}</span>
        <span class="badge">${esc(x.product_type)}</span>
      </div>
      <div class="mono">${esc(x.sku_code)}</div>
      <div style="margin-top:8px">${esc(x.safe_term)}</div>
      <div class="note" style="margin-top:8px">${esc(x.dispatch_action)}</div>
    </div>
  `;
}
...
function renderGrid(items){
  const mapping = {
    'FAST_DISPATCH':'col-fast',
    'CREATIVE_DISPATCH':'col-creative',
    'POSTER_DISPATCH':'col-poster',
    'RESERVE_DISPATCH':'col-reserve',
    'MANUAL_DISPATCH':'col-manual',
  };
  ...
  for (const [group, id] of Object.entries(mapping)) {
    const html = items.filter(x=>x.dispatch_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items = RAW.filter(x=>{
    const hay = [x.title_en, x.sku_code, x.safe_term, x.dispatch_action].join(" ").toLowerCase();
...
fetch("/docs/dispatch_center_v75.json")
  .then(r=>r.json())
  .then(data=>{
    RAW = data.items || [];
