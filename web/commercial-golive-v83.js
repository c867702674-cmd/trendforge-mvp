...
function renderSummary(summary){
  ...
  const parts=[`<div class="card"><div>总上线动作</div><strong>${summary.total || 0}</strong></div>`];
  const byLane=summary.by_golive_lane || {};
...
function cardHtml(x){
  return `<div class="item">
    ...
    <div class="note" style="margin-top:8px">${esc(x.golive_note)}</div>
    <div class="note" style="margin-top:8px"><strong>Operator:</strong> ${esc(x.operator_action)}</div>
    <div class="note" style="margin-top:8px">${esc(x.checklist_text).replace(/\\n/g,'<br>')}</div>
  </div>`;
}
function renderGrid(items){
  const mapping={GO_LIVE_NOW:"col-live",ASSET_FINISH:"col-asset",COMPLIANCE_GATE:"col-compliance",POSTER_FINALIZE:"col-poster",OPERATOR_HOLD:"col-hold"};
  ...
  for(const [group,id] of Object.entries(mapping)){
    const html=items.filter(x=>x.golive_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items=RAW.filter(x=>{
    const hay=[x.title_en,x.sku_code,x.safe_term,x.golive_note,x.operator_action,x.checklist_text].join(" ").toLowerCase();
...
fetch("/docs/commercial_golive_v83.json").then(r=>r.json()).then(data=>{
  RAW=data.items || [];
