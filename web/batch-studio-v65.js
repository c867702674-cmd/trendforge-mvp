...
function renderSummary(summary){
  ...
  const parts = [`<div class="card"><div>总批处理台</div><strong>${summary.total || 0}</strong></div>`];
  const byGroup = summary.by_batch_group || {};
  Object.keys(byGroup).sort().forEach(k=>parts.push(`<div class="card"><div>${esc(k)}</div><strong>${byGroup[k]}</strong></div>`));
...
function cls(group){
  if(group === 'FAST_PUBLISH_BATCH') return 'fast';
  if(group === 'ART_BATCH') return 'art';
  if(group === 'POSTER_BATCH') return 'poster';
  if(group === 'BACKLOG_BATCH') return 'backlog';
  return 'manual';
}
...
function renderGrid(items){
  const mapping = {
    'FAST_PUBLISH_BATCH':'col-fast',
    'ART_BATCH':'col-art',
    'POSTER_BATCH':'col-poster',
    'BACKLOG_BATCH':'col-backlog',
    'MANUAL_BATCH':'col-manual',
  };
  ...
  for (const [group, id] of Object.entries(mapping)) {
    const html = items.filter(x=>x.batch_group===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items = RAW.filter(x=>{
    const hay = [x.title_en, x.sku_code, x.safe_term, x.batch_action].join(" ").toLowerCase();
...
fetch("/docs/batch_studio_v65.json")
  .then(r=>r.json())
  .then(data=>{
    RAW = data.items || [];
  ...
  .catch(err=>{
    document.getElementById("col-fast").innerHTML = `<div class="item">加载失败：${esc(err.message || err)}</div>`;
  });
