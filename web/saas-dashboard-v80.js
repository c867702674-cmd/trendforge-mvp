...
function renderSummary(summary){
  ...
  const parts=[`<div class="card"><div>总工作台</div><strong>${summary.total || 0}</strong></div>`];
  const byLane=summary.by_dashboard_lane || {};
...
function cardHtml(x){
  return `<div class="item">
    ...
    <div class="note" style="margin-top:8px">${esc(x.tags_text)}</div>
  </div>`;
}
function renderGrid(items){
  const mapping={DO_NOW:"col-do",READY_PACK:"col-ready",REVIEW_QUEUE:"col-review",POSTER_LAB:"col-poster",MANUAL_DESK:"col-manual"};
  ...
  for(const [group,id] of Object.entries(mapping)){
    const html=items.filter(x=>x.dashboard_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items=RAW.filter(x=>{
    const hay=[x.title_en,x.sku_code,x.safe_term,x.tags_text].join(" ").toLowerCase();
...
fetch("/docs/saas_dashboard_v80.json").then(r=>r.json()).then(data=>{
  RAW=data.items || [];
  ...
  document.getElementById("col-do").innerHTML=`<div class="item">加载失败：${esc(err.message || err)}</div>`;
});
