...
function renderSummary(summary){
  ...
  const parts=[`<div class="card"><div>总推送执行</div><strong>${summary.total || 0}</strong></div>`];
  const byStatus=summary.by_status || {};
  Object.keys(byStatus).sort().forEach(k=>parts.push(`<div class="card"><div>${esc(k)}</div><strong>${byStatus[k]}</strong></div>`));
  el.innerHTML=parts.join("");
}
function badgeCls(v){
  if(v==="PUSHED") return "pushed";
  if(v==="FAILED") return "failed";
  return "skipped";
}
function cardHtml(x){
  return `<div class="item">
    <strong style="font-size:20px">Execution #${esc(x.execution_job_id)}</strong>
    <div class="meta">
      <span class="badge ${badgeCls(x.push_status)}">${esc(x.push_status)}</span>
      <span class="badge">${esc(x.target_channel)}</span>
    </div>
    <div class="panel"><div class="label">RESPONSE</div>${esc(x.response_text)}</div>
    <div class="panel"><div class="label">PUSHED AT</div>${esc(x.pushed_at)}</div>
  </div>`;
}
...
function applyFilters(){
  ...
  const items=RAW.filter(x=>{
    const hay=[x.target_channel,x.response_text].join(" ").toLowerCase();
...
fetch("/docs/feishu_webhook_v94.json").then(r=>r.json()).then(data=>{
  RAW = data.items || [];
