...
function statusCls(v){ return v==="READY" ? "ready" : "pending"; }
function renderSummary(summary){
  const el=document.getElementById("summary");
  const parts=[`<div class="card"><div>总运营上线项</div><strong>${summary.total || 0}</strong></div>`];
  const byLane=summary.by_ops_lane || {};
...
function cardHtml(x){
  return `<div class="item">
    ...
    <div class="meta">
      ...
      <span class="badge ${statusCls(x.ops_status)}">${esc(x.ops_status)}</span>
...
function renderGrid(items){
  const mapping={PUSH_NOW:"col-push",ASSET_TODO:"col-asset",COMPLIANCE_TODO:"col-compliance",POSTER_TODO:"col-poster",MANUAL_TODO:"col-manual"};
  ...
  for(const [group,id] of Object.entries(mapping)){
    const html=items.filter(x=>x.ops_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items=RAW.filter(x=>{
    const hay=[x.title_en,x.sku_code,x.safe_term,x.blocker_text,x.next_action,x.publish_note].join(" ").toLowerCase();
    if(q && !hay.includes(q)) return false;
    if(priority && x.priority_tier!==priority) return false;
    if(status && x.ops_status!==status) return false;
    if(owner && x.owner_role!==owner) return false;
    return true;
  });
  renderGrid(items);
}
fetch("/docs/launch_ops_v86.json").then(r=>r.json()).then(data=>{
  RAW=data.items || [];
  ...
  document.getElementById("col-push").innerHTML=`<div class="item">加载失败：${esc(err.message || err)}</div>`;
});
