...
function renderSummary(summary){
  ...
  const parts = [`<div class="card"><div>总作战室</div><strong>${summary.total || 0}</strong></div>`];
  const byLane = summary.by_war_lane || {};
...
function cls(lane){
  if(lane === 'DEPLOY_NOW') return 'deploy';
  if(lane === 'ART_COMMAND') return 'art';
  if(lane === 'POSTER_COMMAND') return 'poster';
  if(lane === 'RESERVE_QUEUE') return 'reserve';
  return 'manual';
}
...
function renderGrid(items){
  const mapping = {
    'DEPLOY_NOW':'col-deploy',
    'ART_COMMAND':'col-art',
    'POSTER_COMMAND':'col-poster',
    'RESERVE_QUEUE':'col-reserve',
    'MANUAL_COMMAND':'col-manual',
  };
  ...
  for (const [group, id] of Object.entries(mapping)) {
    const html = items.filter(x=>x.war_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items = RAW.filter(x=>{
    const hay = [x.title_en, x.sku_code, x.safe_term, x.war_action].join(" ").toLowerCase();
...
fetch("/docs/war_room_v68.json")
  ...
  .catch(err=>{
    document.getElementById("col-deploy").innerHTML = `<div class="item">加载失败：${esc(err.message || err)}</div>`;
  });
