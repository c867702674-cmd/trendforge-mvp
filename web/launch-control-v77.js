...
function renderSummary(summary){
  ...
  const parts = [`<div class="card"><div>总发布控制中心</div><strong>${summary.total || 0}</strong></div>`];
  const byLane = summary.by_control_lane || {};
...
function cls(lane){
  if(lane === 'CONTROL_RELEASE') return 'release';
  if(lane === 'CONTROL_CREATIVE') return 'creative';
  if(lane === 'CONTROL_POSTER') return 'poster';
  if(lane === 'CONTROL_RESERVE') return 'reserve';
  return 'manual';
}
  ...
  return `
    <div class="item">
      ...
      <div class="meta">
        ...
        <span class="badge ${cls(x.control_lane)}">${esc(x.control_lane)}</span>
        <span class="badge">${esc(x.product_type)}</span>
      </div>
      <div class="mono">${esc(x.sku_code)}</div>
      <div style="margin-top:8px">${esc(x.safe_term)}</div>
      <div class="note" style="margin-top:8px">${esc(x.control_action)}</div>
    </div>
  `;
}
...
function renderGrid(items){
  const mapping = {
    'CONTROL_RELEASE':'col-release',
    'CONTROL_CREATIVE':'col-creative',
    'CONTROL_POSTER':'col-poster',
    'CONTROL_RESERVE':'col-reserve',
    'CONTROL_MANUAL':'col-manual',
  };
  ...
  for (const [group, id] of Object.entries(mapping)) {
    const html = items.filter(x=>x.control_lane===group).map(cardHtml).join("");
...
function applyFilters(){
  ...
  const items = RAW.filter(x=>{
    const hay = [x.title_en, x.sku_code, x.safe_term, x.control_action].join(" ").toLowerCase();
...
fetch("/docs/launch_control_v77.json")
  .then(r=>r.json())
  .then(data=>{
    RAW = data.items || [];
  ...
  .catch(err=>{
    document.getElementById("col-release").innerHTML = `<div class="item">加载失败：${esc(err.message || err)}</div>`;
  });
