...
echo "Installing TrendForge V110..."
...
app = FastAPI(title="TrendForge Auth V110")
...
async function login(){
  ...
  const res = await fetch("/api/login_v110?email=" + encodeURIComponent(email) + "&pw=" + encodeURIComponent(pw));
...
async function loadMe(){
  ...
  const res = await fetch("/api/me_v110?token=" + encodeURIComponent(token));
  const data = await res.json();
...
# systemd service
############################
cat > /etc/systemd/system/trendforge-auth-v110.service << 'EOF'
[Unit]
Description=TrendForge Auth V110 FastAPI Service
After=network.target

[Service]
User=root
WorkingDirectory=/root/trendforge-mvp/server
ExecStart=/usr/bin/python3 -m uvicorn auth_server_v110:app --host 127.0.0.1 --port 8010
Restart=always
RestartSec=3
...
echo "TrendForge V110 files installed successfully."
